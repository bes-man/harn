...
(harn/studio.py's launch_stage/rerun_workflow), on top of loop.run_stage /
...
from harn import studio, tasks, gitutil, ENV_DIRNAME
...
def test_launch_stage_rejects_unknown_stage(tmp_path):
    root = _repo(tmp_path)
    env = _env(root)
    t = tasks.create_task(env, "Add auth")
    r = studio.launch_stage(env, {"task_id": t.id, "stage": "not_real"})
    assert r["ok"] is False and "unknown stage" in r["error"]


def test_launch_stage_rejects_unknown_task(tmp_path):
    root = _repo(tmp_path)
    env = _env(root)
    r = studio.launch_stage(env, {"task_id": "NOPE", "stage": "execute"})
    assert r["ok"] is False


def test_launch_stage_starts_background_process(tmp_path):
    root = _repo(tmp_path)
    env = _env(root)
    t = tasks.create_task(env, "Add auth")
    with patch("harn.runner.subprocess.Popen", return_value=_fake_popen()), \
         patch("harn.runner.os.kill"):
        r = studio.launch_stage(env, {"task_id": t.id, "stage": "verify", "rerun": True})
        assert r["ok"] is True and r["stage"] == "verify" and r["rerun"] is True
        board = studio.board_payload(env)
        assert board["run"]["task_id"] == t.id
        assert board["run"]["stage"] == "verify"
