...
def reconcile_brief(env_dir: Path, project_root: Path, task) -> str:
    ...
    return (
        ...
        "append it.\n"
        "4. **Service registry** — if this task changed a service's "
        "responsibility, established a new standard, or added a constraint, "
        "refresh that service's file via `save_service` (read_service first, "
        "carry over what's still true). New module → register it.\n"
