...
# Per-step type: an "agent" step (default) or a "command" step that runs a
# shell command, plus that command and where to jump on failure (Phase 2).
...
def parse(env_dir: Path) -> dict:
    ...
    Per node:
      ...
      • `kind`   — "step" (a numbered pipeline step: had a number OR carries a
                   Skills/Tools/Id/Agent/Model/Effort/Temperature/Type/Command/
                   On fail line) or "note" (plain doc section).
      ...
      • `id`, `agent`, `model`, `effort`, `temperature` — from explicit
                   ...
                   see `ensure_ids()`.
      • `type`   — "" (meaning "agent", the default) or "command", from an
                   explicit `Type:` line; any other value is dropped to "".
      • `command`— the shell command for a "command" step, from `Command:`,
                   defaulting to "".
      • `on_fail`— the STABLE ID (not title) of the step to jump to on failure,
                   resolved from an `On fail: <title>` line. Stored as an id so
                   it survives the target step being renamed, exactly like
                   `Id:` itself. Resolution is a SECOND pass done after every
                   node (and its id) is known — a step's `On fail:` line may
