...
def parse(env_dir: Path) -> dict:
    ...
    Per node:
      ...
      • `kind`   — "step" (a numbered pipeline step: had a number OR carries a
                   Skills/Tools line) or "note" (plain doc section).
      • `enabled`— False when the section carries a `Disabled: true` line.
    The `Skills (required: …)` / `Tools: …` lines are lifted into fields. The
    auto-generated skills snapshot block is skipped. `compose()` is the inverse.
    """
    p = env_dir / FILENAME
    if not p.exists():
        write(env_dir)
