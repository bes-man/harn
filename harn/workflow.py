...
from .config import MODEL_STAGES
...
# Explicit mapping to one of loop.py's real, separately-invoked agent turns
# (harn.config.MODEL_STAGES) — NOT auto-detected from the title text. Most
# steps here are prose within ONE turn's prompt (e.g. "Session start"),
# genuinely not a separate invocation; only a step that IS one gets this line,
# and only the studio UI's explicit per-step dropdown ever sets it (never
# guessed from keywords, so renaming a step never silently loses the mapping).
...
def parse(env_dir: Path) -> dict:
    """Parse WORKFLOW.md into an editable structure for the visual editor:
        ...
        {"preamble": "<text before the first ## heading>",
         "nodes": [{"title", "body", "kind", "enabled", "required", "tools", "stage"}]}
    ...
    Per node:
      ...
      • `kind`   — "step" (a numbered pipeline step: had a number OR carries a
                   Skills/Tools/Stage line) or "note" (plain doc section).
      • `enabled`— False when the section carries a `Disabled: true` line.
      • `stage`  — from an explicit `Stage: <name>` line: "" (undecided — the
                   studio UI falls back to a best-effort title-keyword guess),
                   "none" (explicitly NOT one of harn run's real turns — opts
                   out of the guess), or one of harn.config.MODEL_STAGES (this
                   step IS one of harn run's real, separately-invoked agent
                   turns — unlocking a per-step model override and Run/Rerun
                   in the studio UI). This module never guesses — only the
                   studio UI's explicit per-step dropdown ever sets this, so a
                   confirmed mapping survives the step being renamed later.
    The `Skills (required: …)` / `Tools: …` / `Stage: …` lines are lifted into
    fields. The auto-generated skills snapshot block is skipped. `compose()` is
    the inverse.
    """
    p = env_dir / FILENAME
    if not p.exists():
    ...
    for line in lines:
        ...
        if h:
            ...
            cur = {"title": (m.group(2).strip() if m else raw),
                   "required": [], "tools": [], "enabled": True, "stage": "",
        ...
        if tl:
            ...
            continue
        stg = _STAGE_RE.match(line)
        if stg:
            cur["_decl"] = True
            val = stg.group(1).strip().lower()
            # "none" is an explicit, persisted opt-out (never auto-detect for
            # this step) — distinct from "" (undecided: the UI falls back to
            # guessing from the title). Anything else unrecognized -> undecided.
            cur["stage"] = val if (val == "none" or val in MODEL_STAGES) else ""
            continue
        body.append(line)
    _flush()
...
def compose(env_dir: Path, parsed: dict) -> str:
    ...
    for n in parsed.get("nodes", []):
        ...
        stage = (n.get("stage") or "").strip().lower()
        if kind == "step" and (stage == "none" or stage in MODEL_STAGES):
